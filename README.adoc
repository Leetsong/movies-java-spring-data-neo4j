...
. Run `npm install` in `src/main/js`.
. Run `umi build` in `src/main/js`.
. Run the project with `mvn spring-boot:run` in the root folder`.
...
* Frontend:                 bootstrap, https://umijs.org/[umi], https://antv.alipay.com/zh-cn/g6/1.x/index.html/[AntV-G6]
